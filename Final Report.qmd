...
We will examine the average value of each metric for a given year across all countries in the data set.
...
We calcualted our variances for the response, fitted, and residual values to analyze how well our linear model fits the data. A lower variance in fitted values indicate a better model. 
```{r}
...
predicted <- lin_data |>
  ...
  geom_point() +
  labs(x="Mean Years in School", y="Predicted Mean Life Expectancy", title="Years in School vs Predicted Life Expectancy") +
  scale_y_continuous(limits=c(40, 85))

actual <- actual + scale_y_continuous(limits=c(40, 85))
...
We were interested in seeing how our model performs over many different data sets. Thus, we simulated 1000 data sets
