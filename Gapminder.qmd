...
relationship_plot <- joined_data |>
  ggplot(aes(x= years_in_school, y = life_expectancy, alpha = 0.15)) + geom_point(color = "darkcyan", show.legend = FALSE) + xlab("Years in school for 15-24 \n year old women") + ylab("Life expectancy") + labs(title = "Life Expectancy and Years in School")
...
time_plot <- joined_data |>
  group_by(year) |>
  summarize(mean_life = mean(life_expectancy), mean_school = mean(years_in_school, na.rm = TRUE)) |>
  mutate(std_life = rescale_01(mean_life), std_school = rescale_01(mean_school)) |>
  ungroup() |>
  ggplot() + geom_line(aes(x= year, y = std_life, group = 1, color = "darkseagreen")) + geom_line(aes(x= year, y = std_school, group = 1, color = "darkorchid4")) + scale_color_manual(values = c("darkseagreen", "darkorchid4"), labels = c("Life Expectancy (rescaled)", "Years in School \nfor 15-24 y/o Women (rescaled)")) + labs(color = "Rescaled Values", x = "Year", y = "", subtitle = "Rescaled Values", title = "General Life Expectancy and \nYears in School Over Time") + scale_x_discrete(breaks = seq(1970, 2015, 5))
time_plot
